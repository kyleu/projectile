...
object BigRowQueries extends BaseQueries[BigRow]("bigRow", "big") {
  ...
  def insert(model: BigRow) = new InsertNoPk(model)
  def insertBatch(models: Seq[BigRow]) = new InsertBatch(models)
  ...
  override def fromRow(row: Row) = BigRow(
    id = LongType(row, "id"),
