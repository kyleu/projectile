...
import models.graphql.b.BottomRowSchema
...
object TopRowSchema extends GraphQLSchemaHelper("topRow") {
  ...
  private[this] def getByPrimaryKeySeq(c: GraphQLContext, idSeq: Seq[UUID]) = c.getInstance[services.t.TopRowService].getByPrimaryKeySeq(c.creds, idSeq)(c.trace)
  ...
  val queryFields = fields(
    unitField(name = "topRow", desc = None, t = OptionType(topRowType), f = (c, td) => {
      c.ctx.getInstance[services.t.TopRowService].getByPrimaryKey(c.ctx.creds, c.arg(topRowIdArg))(td)
    }, topRowIdArg),
    unitField(name = "topRowSeq", desc = None, t = ListType(topRowType), f = (c, td) => {
      c.ctx.getInstance[services.t.TopRowService].getByPrimaryKeySeq(c.ctx.creds, c.arg(topRowIdSeqArg))(td)
    }, topRowIdSeqArg),
    unitField(name = "topRowSearch", desc = None, t = topRowResultType, f = (c, td) => {
      runSearch(c.ctx.getInstance[services.t.TopRowService], c, td).map(toResult)
    }, queryArg, reportFiltersArg, orderBysArg, limitArg, offsetArg),
    unitField(name = "topsByT", desc = None, t = ListType(topRowType), f = (c, td) => {
      c.ctx.getInstance[services.t.TopRowService].getByT(c.ctx.creds, c.arg(topRowTArg))(td)
    }, topRowTArg),
    unitField(name = "topsByTSeq", desc = None, t = ListType(topRowType), f = (c, td) => {
      c.ctx.getInstance[services.t.TopRowService].getByTSeq(c.ctx.creds, c.arg(topRowTSeqArg))(td)
    }, topRowTSeqArg)
  )
  ...
  val topRowMutationType = ObjectType(
    ...
    fields = fields(
      unitField(name = "create", desc = None, t = OptionType(topRowType), f = (c, td) => {
        c.ctx.getInstance[services.t.TopRowService].create(c.ctx.creds, c.arg(dataFieldsArg))(td)
      }, dataFieldsArg),
      unitField(name = "update", desc = None, t = OptionType(topRowType), f = (c, td) => {
        c.ctx.getInstance[services.t.TopRowService].update(c.ctx.creds, c.arg(topRowIdArg), c.arg(dataFieldsArg))(td).map(_._1)
      }, topRowIdArg, dataFieldsArg),
      unitField(name = "remove", desc = None, t = topRowType, f = (c, td) => {
        c.ctx.getInstance[services.t.TopRowService].remove(c.ctx.creds, c.arg(topRowIdArg))(td)
      }, topRowIdArg)
    )
  )
