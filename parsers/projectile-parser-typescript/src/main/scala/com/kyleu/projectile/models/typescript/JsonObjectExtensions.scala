...
object JsonObjectExtensions {
  class RichJsonObject(o: JsonObject) {
    ...
    def typReq(k: String = "type") = FieldTypeRequired(t = typOrAny(k), r = o.apply("questionToken").isEmpty)
    ...
    def tParams(k: String = "typeParameters") = o.apply(k).map(extract[Seq[JsonObject]]).getOrElse(Nil).map(MethodHelper.getTParam)
    ...
    def memberFields(k: String = "members") = o.apply(k).map(extract[Seq[JsonObject]]).getOrElse(Nil).map(MethodHelper.getParam)
