...
object ExportModelReference {
  case class Ref(r: ExportModelReference, f: ExportField, src: ExportModel, tf: ExportField)
  ...
  def transformedReferences(config: ExportConfiguration, model: ExportModel) = validReferences(config, model).flatMap { r =>
    ...
    model.getFieldOpt(r.tgt).flatMap(f => src.getFieldOpt(r.srcCol).map(tf => Ref(r, f, src, tf)))
...
case class ExportModelReference(name: String, propertyName: String = "", srcTable: String, srcCol: String, tgt: String, notNull: Boolean)
