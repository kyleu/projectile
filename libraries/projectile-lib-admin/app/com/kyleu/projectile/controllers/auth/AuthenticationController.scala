...
class AuthenticationController @javax.inject.Inject() (
  ...
  def signInForm = withoutSession("form") { implicit request => _ =>
  ...
  def authenticateCredentials = withoutSession("authenticate") { implicit request => implicit td =>
    UserForms.signInForm.bindFromRequest.fold(
      form => Future.successful(BadRequest(signin(form, app.cfg(u = request.identity)))),
      credentials => {
  ...
  def signOut = withSession("signout") { implicit request => _ =>
  ...
  private[this] def signin(form: Form[Credentials], cfg: UiConfig)(implicit flash: Flash): HtmlFormat.Appendable = {
