...
class ScheduledTaskRunController @javax.inject.Inject() (
  ...
  def createForm = withSession("create.form", admin = true) { implicit request => implicit td =>
    ...
    Future.successful(Ok(com.kyleu.projectile.views.html.admin.task.scheduledTaskRunForm(
      app.cfgAdmin(u = request.identity, "task", "scheduled_task_run", "Create"), ScheduledTaskRun.empty(), "New Scheduled Task Run", cancel, call, isNew = true, debug = app.config.debug
    )))
  }
    ...
    withSession("list", admin = true) { implicit request => implicit td =>
      ...
      searchWithCount(q, orderBys, limit, offset).map(r => renderChoice(t) {
        case MimeTypes.HTML => r._2.toList match {
          case model :: Nil => Redirect(com.kyleu.projectile.controllers.admin.task.routes.ScheduledTaskRunController.view(model.id))
          case _ => Ok(com.kyleu.projectile.views.html.admin.task.scheduledTaskRunList(app.cfgAdmin(u = request.identity, "task", "scheduled_task_run"), Some(r._1), r._2, q, orderBy, orderAsc, limit.getOrElse(100), offset.getOrElse(0)))
        }
  ...
  def view(id: UUID, t: Option[String] = None) = withSession("view", admin = true) { implicit request => implicit td =>
    ...
    notesF.flatMap(notes => auditsF.flatMap(audits => modelF.map {
      case Some(model) => renderChoice(t) {
        case MimeTypes.HTML => Ok(com.kyleu.projectile.views.html.admin.task.scheduledTaskRunView(app.cfgAdmin(u = request.identity, "task", "scheduled_task_run", model.id.toString), model, notes, audits, app.config.debug))
  ...
  def editForm(id: UUID) = withSession("edit.form", admin = true) { implicit request => implicit td =>
    ...
    svc.getByPrimaryKey(request, id).map {
      case Some(model) => Ok(
        com.kyleu.projectile.views.html.admin.task.scheduledTaskRunForm(app.cfgAdmin(u = request.identity, "task", "scheduled_task_run", "Edit"), model, s"Scheduled Task Run [$id]", cancel, call, debug = app.config.debug)
      )
      case None => NotFound(s"No ScheduledTaskRun found with id [$id]")
    }
  }
