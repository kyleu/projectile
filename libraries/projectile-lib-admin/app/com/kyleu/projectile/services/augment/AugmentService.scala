...
class AugmentListService[Rsp](errResponse: Throwable => Option[Rsp]) extends Logging {
  private[this] var map = Map.empty[Class[_], (Seq[Any], Map[String, Seq[String]], UiConfig, TraceData) => Future[(Option[Rsp], Map[Any, Option[Rsp]])]]

  def register[T](f: (Seq[T], Map[String, Seq[String]], UiConfig, TraceData) => Future[(Option[Rsp], Map[T, Option[Rsp]])])(implicit tag: ClassTag[T]) = {
    map = map + (tag.runtimeClass -> f.asInstanceOf[(Seq[Any], Map[String, Seq[String]], UiConfig, TraceData) => Future[(Option[Rsp], Map[Any, Option[Rsp]])]])
  }
