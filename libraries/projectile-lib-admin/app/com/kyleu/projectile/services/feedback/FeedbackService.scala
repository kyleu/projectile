...
class FeedbackService @javax.inject.Inject() (val db: JdbcDatabase, override val tracing: TracingService)(implicit ec: ExecutionContext) extends ModelServiceHelper[Feedback]("feedback") {
  ...
  def countByAuthorEmail(creds: Credentials, authorEmail: String)(implicit trace: TraceData) = traceF("count.by.authorEmail") { td =>
    db.queryF(FeedbackQueries.CountByAuthorEmail(authorEmail))(td)
  }
  def getByAuthorEmail(creds: Credentials, authorEmail: String, orderBys: Seq[OrderBy] = Nil, limit: Option[Int] = None, offset: Option[Int] = None)(implicit trace: TraceData) = traceF("get.by.authorEmail") { td =>
    db.queryF(FeedbackQueries.GetByAuthorEmail(authorEmail, orderBys, limit, offset))(td)
  }
  def getByAuthorEmailSeq(creds: Credentials, authorEmailSeq: Seq[String])(implicit trace: TraceData) = if (authorEmailSeq.isEmpty) {
    Future.successful(Nil)
  } else {
  ...
  def countByAuthorId(creds: Credentials, authorId: UUID)(implicit trace: TraceData) = traceF("count.by.authorId") { td =>
    db.queryF(FeedbackQueries.CountByAuthorId(authorId))(td)
  }
  def getByAuthorId(creds: Credentials, authorId: UUID, orderBys: Seq[OrderBy] = Nil, limit: Option[Int] = None, offset: Option[Int] = None)(implicit trace: TraceData) = traceF("get.by.authorId") { td =>
    db.queryF(FeedbackQueries.GetByAuthorId(authorId, orderBys, limit, offset))(td)
  }
  def getByAuthorIdSeq(creds: Credentials, authorIdSeq: Seq[UUID])(implicit trace: TraceData) = if (authorIdSeq.isEmpty) {
    Future.successful(Nil)
  } else {
  ...
  // Mutations
  def insert(creds: Credentials, model: Feedback)(implicit trace: TraceData) = traceF("insert") { td =>
