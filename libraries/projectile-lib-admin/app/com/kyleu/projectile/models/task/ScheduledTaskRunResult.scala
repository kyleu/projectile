...
object ScheduledTaskRunResult {
  implicit val jsonEncoder: Encoder[ScheduledTaskRunResult] = (r: ScheduledTaskRunResult) => io.circe.Json.obj(
  ...
  implicit val jsonDecoder: Decoder[ScheduledTaskRunResult] = (c: io.circe.HCursor) => for {
    ...
    results <- c.downField("results").as[Seq[ScheduledTaskRun]]
    durationMs <- c.downField("durationMs").as[Int]
    occurred <- c.downField("occurred").as[LocalDateTime]
  } yield ScheduledTaskRunResult(filters, orderBys, totalCount, paging, results, durationMs, occurred)
