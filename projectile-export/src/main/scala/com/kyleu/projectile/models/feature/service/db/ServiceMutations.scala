  ...
  def mutations(config: ExportConfiguration, model: ExportModel, file: ScalaFile) = {
    if (model.pkFields.nonEmpty) {
      ...
      if (model.foreignKeys.nonEmpty && (!config.project.flags(ProjectFlag.NoBulk))) {
        file.add()
        file.add(s"""def updateBulk(creds: Credentials, pks: Seq[Seq[Any]], fields: Seq[DataField], $conn)$trace = $editCheck{""", 1)
        file.add(s"db.executeF(${model.className}Queries.updateBulk(pks, fields), conn)(trace).map { x =>", 1)
