...
object ThriftServiceFile {
  def export(config: ExportConfiguration, svc: ExportService) = {
    ...
    file.add(") {")
    file.add(s"""def mkServicePerEndpoint(url: String): ${thriftServiceCanonicalName}.ReqRepServicePerEndpoint = {""", 1)
    file.add(s"""ThriftMux.client.servicePerEndpoint[${thriftServiceCanonicalName}.ReqRepServicePerEndpoint](url, "svc.className")""")
    file.add("}", -1)
    file.add(s"""def injectTraceDataToHeaders(options: Options)(headers: Map[String, String], td: TraceData): Map[String, String] = {""", 1)
