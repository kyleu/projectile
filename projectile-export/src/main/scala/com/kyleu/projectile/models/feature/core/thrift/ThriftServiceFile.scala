...
object ThriftServiceFile {
  ...
  private[this] def addMethods(thriftServiceCanonicalName: String, config: ExportConfiguration, file: ScalaFile, svc: ExportService) = {
    svc.methods.foreach { method =>
      ...
      file.add(s"""def ${method.name}($args)(implicit td: TraceData, headers: Map[String, String] = Map.empty): Future[$s] = trace("${method.name}") { _ =>""", 1)
      ...
      file.add(s"val _requestWithHeaders = headers.foldLeft(_request)((acc, kv) => acc.setHeader(kv._1, kv._2))")
