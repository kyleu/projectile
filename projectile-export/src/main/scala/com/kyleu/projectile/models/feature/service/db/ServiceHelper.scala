...
object ServiceHelper {
  def writeSearchFields(model: ExportModel, file: ScalaFile, queriesFile: String, trace: String, searchArgs: String) = {
    file.add(s"""override def countAll(creds: Credentials, filters: Seq[Filter] = Nil)$trace = checkPerm(creds, "view") {""", 1)
    file.add(s"""traceF("get.all.count")(td => db.queryF($queriesFile.countAll(filters))(td))""")
    file.add("}", -1)
    file.add(s"""override def getAll(creds: Credentials, $searchArgs)$trace = checkPerm(creds, "view") {""", 1)
    file.add(s"""traceF("get.all")(td => db.queryF($queriesFile.getAll(filters, orderBys, limit, offset))(td))""")
    file.add("}", -1)
    file.add()
    file.add("// Search")
    file.add(s"""override def searchCount(creds: Credentials, q: Option[String], filters: Seq[Filter] = Nil)$trace = checkPerm(creds, "view") {""", 1)
    ...
    file.add(s""")$trace = checkPerm(creds, "view") {""", 1)
  ...
  def addGetters(config: ExportConfiguration, model: ExportModel, file: ScalaFile) = {
    ...
    model.pkFields match {
      ...
      case field :: Nil =>
        ...
        file.add(s"""def getByPrimaryKey(creds: Credentials, $colProp: ${field.scalaType(config)})$td = checkPerm(creds, "view") {""", 1)
        ...
        file.add(s"""def getByPrimaryKeySeq(creds: Credentials, $seqArgs)$td = checkPerm(creds, "view") {""", 1)
      ...
      case fields => // multiple columns
        ...
        file.add(s"""def getByPrimaryKey(creds: Credentials, $colArgs)$td = checkPerm(creds, "view") {""", 1)
        file.add(s"""traceF("get.by.primary.key")(td => db.queryF(${model.className}Queries.getByPrimaryKey($queryArgs))(td))""")
        file.add("}", -1)

        file.add(s"""def getByPrimaryKeySeq(creds: Credentials, pkSeq: Seq[$tupleTyp])$td = checkPerm(creds, "view") {""", 1)
  ...
  private[this] def addRelationMethodsToFile(config: ExportConfiguration, model: ExportModel, file: ScalaFile, col: String) = {
    ...
    file.add(s"""def countBy$propCls(creds: Credentials, $propId: ${field.scalaType(config)})(implicit trace: TraceData) = checkPerm(creds, "view") {""", 1)
    ...
    file.add(s"""def getBy$propCls($fkArgs)(implicit trace: TraceData) = checkPerm(creds, "view") {""", 1)
    ...
    file.add(s"""def getBy${propCls}Seq($fkSeqArgs)(implicit trace: TraceData) = checkPerm(creds, "view") {""", 1)
