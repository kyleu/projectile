...
object GraphQLOutputTranslations {
  ...
  def scalaImport(cfg: GraphQLExportConfig, t: Type, nameMap: Map[String, ClassName]): Option[(Seq[String], String)] = t match {
    case OptionType(x) => scalaImport(cfg, x, nameMap)
    case ListType(x) => scalaImport(cfg, x, nameMap)
    case EnumType(n, _, _, _, _) => Some(nameMap.get(n).map(_.pkg.toSeq).getOrElse(cfg.pkgFor(n)) -> n)
    case _ => t.namedType.name match {
      case "DateTime" | "Date" | "Time" => Some(Seq("java", "time") -> ("Zoned" + t.namedType.name))
      case "UUID" => Some(Seq("java", "util") -> t.namedType.name)
