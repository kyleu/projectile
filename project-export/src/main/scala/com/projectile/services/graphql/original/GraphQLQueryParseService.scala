...
class GraphQLQueryParseService(cfg: GraphQLExportConfig, schema: Schema[_, _]) {
  ...
  def export() = {
    ...
    val inputFiles = doc.definitions.flatMap {
      case x: InputObjectTypeDefinition => GraphQLInputServiceOriginal.inputFile(cfg, x, nameMap, schema, enumNames)
      case _ => None
    }
    ...
    outFiles.foreach { f =>
      val out = output / f.dir.mkString("/")
      out.createIfNotExists(createParents = true)
      out.write(f.rendered.content)
    }
  }
