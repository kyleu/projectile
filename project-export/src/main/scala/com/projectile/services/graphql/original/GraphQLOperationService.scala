...
object GraphQLOperationService {
  ...
  def opFile(cfg: GraphQLExportConfig, n: String, d: OperationDefinition, nameMap: Map[String, ClassName], schema: Schema[_, _]) = {
    ...
    val rp = if (cfg.providedPrefix.isEmpty) { Seq("models") } else { cfg.providedPrefix }
  ...
  private[this] def addVariables(
    providedPrefix: Seq[String], file: ScalaFile, variables: Seq[VariableDefinition], nameMap: Map[String, ClassName]
    ...
    val args = variables.map { v =>
      ...
      nameMap.get(typName).foreach(x => file.addImport(x.pkg, x.cn))
