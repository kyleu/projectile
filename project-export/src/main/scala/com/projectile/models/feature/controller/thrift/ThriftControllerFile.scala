...
object ThriftControllerFile {
  def export(config: ExportConfiguration, service: ExportService) = {
    val file = ScalaFile(path = OutputPath.ServerSource, config.applicationPackage ++ service.pkg :+ "controllers", service.className + "Controller")
    ...
    file.add(s"""private[this] val rc = controllers.admin.thrift.${service.propertyName}.routes.${service.className}Controller""")
    file.add()
    file.add("""def list = withSession("list", admin = true) { implicit request => implicit td =>""", 1)
    file.add(s"Future.successful(Ok(views.html.admin.thrift.${service.propertyName}(request.identity)))")
    file.add("}", -1)

    service.methods.foreach(m => addMethod(service, m, config, file))
    addHelpers(service, file)
  ...
  private[this] def addMethod(svc: ExportService, m: ExportMethod, config: ExportConfiguration, file: ScalaFile) = {
    ...
    if (argNames.isEmpty) {
      ...
      file.add(postCall, 1)
      m.args.foreach { arg =>
        val argRootType = ThriftFileHelper.declarationForField(config, arg)
        val argType = if (arg.notNull) { argRootType } else { s"Option[$argRootType]" }
  ...
  private[this] def addHelpers(svc: ExportService, file: ScalaFile) = {
    file.add()
