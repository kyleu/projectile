...
import models.database.schema.{EnumType, Procedure, ProcedureParam}
import services.database.query.{JdbcHelper, JdbcRow, QueryTranslations}
...
object MetadataProcedures extends Logging {
  def getProcedures(metadata: DatabaseMetaData, catalog: Option[String], schema: Option[String]) = {
    ...
    new JdbcRow.Iter(rs).map(procedureFromRow).toList.sortBy(_.name)
  }
  ...
  def getProcedureDetails(metadata: DatabaseMetaData, catalog: Option[String], schema: Option[String], procedure: Procedure, enums: Seq[EnumType]) = try {
    ...
    val columns = new JdbcRow.Iter(rs2).map(row => columnFromRow(row, enums)).toList.sortBy(_._1).map(_._2)
  ...
  private[this] def procedureFromRow(row: JdbcRow) = {
  ...
  private[this] def columnFromRow(row: JdbcRow, enums: Seq[EnumType]) = {
