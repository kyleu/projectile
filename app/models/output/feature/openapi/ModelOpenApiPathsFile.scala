...
import models.database.schema.ForeignKey
import models.export.{ExportEnum, ExportField, ExportModel}
import models.output.OutputPath
import models.output.file.JsonFile

object ModelOpenApiPathsFile {
  def export(model: ExportModel, enums: Seq[ExportEnum], solo: Boolean) = {
    val file = JsonFile(path = OutputPath.OpenAPIJson, dir = "paths" +: model.pkg, model.propertyName)
    file.add("{", 1)
